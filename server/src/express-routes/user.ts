...
userRouter.post('/', (request: express.Request, response: express.Response) => {
    ...
    if (!params) return response.status(400).send(logger.logResponse('Error, Bad params, for CREATE user'));
...
userRouter.get('/:user_id', (request: express.Request, response: express.Response) => {
    ...
    if (!user_id) return response.status(400).send(logger.logResponse('Error, Bad params, for GET user'));
...
userRouter.put('/:user_id', (request: express.Request, response: express.Response) => {
    ...
    if (!params || !user_id) return response.status(400).send(logger.logResponse('Error, Bad params, for UPDATE user'));
...
userRouter.delete('/:user_id', (request: express.Request, response: express.Response) => {
    var user_id = request.params.user_id;
    if (!user_id) return response.status(400).send(logger.logResponse('Error, Bad params, for DELETE user'));
